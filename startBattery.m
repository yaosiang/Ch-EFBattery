...
try
  ...
  msg = double(['進行第 1 / ', num2str(nTasks), ' 個實驗，按任意鍵開始']);
  showCenteredMessage(windowPtr, msg, WhiteIndex(screenNumber));
  KbWait([], 2);
  ...
  for i = 1:nTasks
    ...
    if i ~= nTasks
      msg = double(['進行第 ', num2str(i + 1), ' / ', num2str(nTasks), ' 個實驗，按任意鍵繼續']);
    else
      msg = double('所有實驗已完成，感謝你的參與！按任意鍵繼續');
    end
    showCenteredMessage(windowPtr, msg, WhiteIndex(screenNumber));
    KbWait([], 2);
  end

  msg = double('請你找主試者，謝謝！');
  showCenteredMessage(windowPtr, msg, WhiteIndex(screenNumber));
  KbWait([], 2);
